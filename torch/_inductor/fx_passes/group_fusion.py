...
class GroupFusionPass:
    ...
    nodes to be fused.
    """
    def __init__(self, pattern, pair_check, replacement_fn, extra_check=lambda m: True, prevent_match_across_mutations=False):
        """
        Args:
            ...
            if target_node not in seen_nodes:
                ...
                for j in range(i + 1, len(target_nodes)):
                    if target_nodes[j] in seen_nodes:
                        continue
                    can_be_fused = self.pair_check(target_node, target_nodes[j]) and get_mutation_region_id(graph, target_node) == get_mutation_region_id(graph, target_nodes[j])
                    if (
                        can_be_fused
...
layer_norm_fusion_pass = GroupFusionPass(
    ...
    extra_check=lambda m: config_flag("split_cat_fx_passes")(m) and layer_norm_extra_check(m),
